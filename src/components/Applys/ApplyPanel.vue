<template>
        ...
        <div class="col-md-10 col-sm-10">
            ...
            <div class="box box-primary">
                <div class="box-header with-border">
                    <div class="row">
                        ...
                        <div class="col-md-4  col-sm-4">
                            <button  class="btn btn-sm"  :class="[canPrint == 1 ? 'btn-info' : '']"  style="padding: 4px 54px; margin-top: 20px;"  v-if="subAmount > 0" @click="PrintBillOneChoice">部门打印</button>
                            <button  class="btn btn-sm"  :class="[canPrint == 1 ? 'btn-success' : '']"  style="padding: 4px 54px; margin: 10px 0px ;" @click="PrintBillOne">全部打印</button>
                        </div>
                    </div>
            ...
            <div style="display: none" id="testDeliveryOne"></div>
...
<script>
    ...
    import PrintSave from '../../components/Applys/PrintSave.vue'
    import ApplyCheck from '../../components/Applys/ApplyCheck.vue'
    ...
    export default {
        ...
        watch: {
            nxDepId: function (newVal) {
                this.nxDepId = newVal;
                this.currPage = 1; this.nxDepId = newVal;
                this.subAmount = 0;
                this.currPage = 1;
        ...
        methods: {
            PrintBillOneChoice(){
                ...
                if(canSave){
                    ...
                    this.showType = "print";
                    window.open("PrintChoice.html?id=" + this.nxDepId + '&printName=ApplyPanel&depName='
                        + this.depName);
                }else{
                    $('#modal_check').show()
                }
            ...
            PrintBillOneMain: function () {
                ...
                if (totalPriPage <= 1) {
                    console.log("yiyeyeyeyeyeeyeyyeyeeyyeyey")
                    var applyBody = `<div id="page1" class="applyBody" ></div>`
                    $('#testDeliveryOne').append(applyBody);
                    ...
                    for (var j = 0; j < arr.length; j++) {
                        var oneItem = `<div class="oneItem" style="float: left;width: 100%; border-bottom: 1px solid gray;
                                                                        line-height: 22px; text-align: center;font-size: 12px;"></div>`
                        ...
                        var nxDoPrice = `<div style="word-break:keep-all;box-sizing: border-box;float: left; width: 15%; border-right: 1px solid gray; text-align: center;  dding-right: 10px;" >` + arr[j]['nxDoPrice'] + `</div>`
                    ...
                    for (var i = 1; i <= totalPriPage; i++) {
                        console.log("dupoduoduoudodududoudoodyeyyyyeyyeey")
                        ...
                        for (var j = i * totalGoods - totalGoods; j < i * totalGoods; j++) {
                            if (j < arrLen) {
                                ...
                                console.log("iiiii" + i +"=" + Number(arr[j].nxDoSubtotal));
                                console.log("iiiii" + i +"=" + pageSubtotal);
            ...
            getOrderPageData: function (e) {
                var data = "page=" + this.currPage + "&limit=" + this.limit + "&depFatherId=" + parseInt(this.nxDepId) + "&gbDepFatherId=" + parseInt(this.gbDepId)
                    + "&resFatherId=" + parseInt(this.resFatherId);
                this.bus.$emit('loading', true);
